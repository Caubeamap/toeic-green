...
async function main() {
  ...
  for (const u of [...imageSet, ...audioSet]) {
    ...
    if (u.includes('study4.com')) {
      ...
      u.includes('.r2.dev') ||
      (process.env.R2_PUBLIC_URL && u.includes(process.env.R2_PUBLIC_URL))
    ) {
      category = 'r2';
  ...
  console.log(
    '  Pointing to Study4.com (Not migrated):',
    classification.study4,
  );
  ...
  console.log(
    '  Pointing to Local "images/toeic" path:',
    classification.local_toeic,
  );
  ...
  const localToeicDir = path.join(
    __dirname,
    '../../..',
    'front-end/public/images/toeic',
  );
  if (fs.existsSync(localToeicDir)) {
    const localFiles = fs
      .readdirSync(localToeicDir)
      .filter((f) => fs.statSync(path.join(localToeicDir, f)).isFile());
    console.log(
    ...
    console.log(
      `  Number of local files referenced/used in DB: ${matchedFiles} / ${localFiles.length}`,
    );
    console.log(
      `  Number of local files NOT referenced in DB (dead weight): ${localFiles.length - matchedFiles}`,
    );
